...
class Inhabitant {
    ...
    description() {
        return ['name', 'gender', 'species', 'saying', 'legs', 'tail', 'friends'].map(property => `${property}: ${Array.isArray(this[property]) && this[property].length === 0 ? 'none' : this[property]}`).join(delimiter);
    }
}
